...
// v132: GLOBAL physical box-number uniqueness across the whole shipment.
// The same physical box number/range may never exist in two completed work groups, even across different center tabs.
function expandPhysicalBoxNosV132(label,count){
  const s=String(label??'').trim().replace(/[～〜–—−－]/g,'~').replace(/\s+/g,'').toUpperCase(),cnt=Math.max(1,Math.floor(Number(count)||1));if(!s)return [];
  const m=s.match(/^(\d+)(?:[~\-](\d+))?$/);if(!m)return ['S:'+s];
  let a=Number(m[1]),b=m[2]?Number(m[2]):(a+cnt-1);if(b<a)[a,b]=[b,a];if(b-a>10000)return ['S:'+s];
  const out=[];for(let n=a;n<=b;n++)out.push('N:'+n);return out;
}
function sanitizeGlobalBoxNoUniqueV132(state){
  if(!state||typeof state!=='object'||!Array.isArray(state.palletStates))return {state:state||{},changed:false,removed:0};
  const out=cloneJsonV82(state)||{},tombs=mergeTombstonesV82(out.boxTombstonesV82,{});out.boxTombstonesV82=tombs;
  const desiredRoot='box-1789795559024-h5wcc',desiredSig=['R245545710001:14','R223850540001:2','R216735080005:24','R236115740008:1'].sort().join('|');
  const txt=v=>String(v??'').trim(),root=id=>{const s=txt(id);const m=s.match(/^(box-\d+-[A-Za-z0-9_-]+)(?:::|$)/);return m?m[1]:s};
  const rev=g=>Number(g&&g.boxRevisionV82||0),updated=g=>Math.max(Number(g&&g.boxUpdatedAtV82||0),Number(g&&g.boxCompletedAtV82||0)),created=g=>Number(g&&g.boxCompletedAtV82)||Number.MAX_SAFE_INTEGER,seq=g=>{const x=Number(g&&g.seq);return x>0?x:Number.MAX_SAFE_INTEGER};
  const desired=g=>{if(root(g&&g.id)===desiredRoot)return true;const nums=expandPhysicalBoxNosV132(g&&g.boxNo,g&&g.boxCount);if(!nums.includes('N:374'))return false;const sig=(Array.isArray(g&&g.items)?g.items:[]).map(i=>`${txt(i&&i.barcode)}:${Number(i&&i.totalQty)||Number(i&&i.perBoxQty)||0}`).sort().join('|');return sig===desiredSig};
  const penalty=g=>g&&g.recoveredSecondTubV121?10:(g&&g.recoveredV121?5:0);
  const candidates=[],raws=new Map();let sheetIndex=0;
  for(const pair of out.palletStates){
    if(!Array.isArray(pair)||pair.length<2||!pair[0]||!pair[1]){sheetIndex++;continue}const sheet=String(pair[0]),raw=cloneJsonV82(pair[1])||{},byId=new Map(),free=[];
    for(const g0 of (Array.isArray(raw.completed)?raw.completed:[])){if(!g0)continue;const g=cloneJsonV82(g0),id=txt(g.id);if(!id){free.push(g);continue}const cur=byId.get(id);if(!cur||rev(g)>rev(cur)||(rev(g)===rev(cur)&&updated(g)>updated(cur)))byId.set(id,g)}
    raw.completed=[];raws.set(sheet,raw);
    for(const g of [...byId.values(),...free]){const tr=Number(tombs&&tombs[sheet]&&tombs[sheet][txt(g.id)]||0);if(tr&&tr>rev(g))continue;candidates.push({sheet,sheetIndex,g,keys:expandPhysicalBoxNosV132(g.boxNo,g.boxCount)})}sheetIndex++;
  }
  candidates.sort((a,b)=>{const ap=desired(a.g)?0:1,bp=desired(b.g)?0:1;if(ap!==bp)return ap-bp;const pe=penalty(a.g)-penalty(b.g);if(pe)return pe;if(a.sheet===b.sheet){const q=seq(a.g)-seq(b.g);if(q)return q}const ct=created(a.g)-created(b.g);if(ct)return ct;const si=a.sheetIndex-b.sheetIndex;if(si)return si;const sq=seq(a.g)-seq(b.g);if(sq)return sq;return txt(a.g&&a.g.id).localeCompare(txt(b.g&&b.g.id))});
  const used=new Map(),kept=new Map(),dropped=[];
  for(const c of candidates){const hit=c.keys.find(k=>used.has(k));if(hit){dropped.push(c);continue}if(!kept.has(c.sheet))kept.set(c.sheet,[]);kept.get(c.sheet).push(c.g);for(const k of c.keys)used.set(k,c)}
  let fence=Math.max(Date.now(),Number(out.stateRevisionV78||0));for(const rows of Object.values(tombs||{}))for(const r of Object.values(rows||{}))fence=Math.max(fence,Number(r||0));for(const c of candidates)fence=Math.max(fence,rev(c.g));fence++;
  for(const d of dropped){const id=txt(d.g&&d.g.id);if(!id)continue;const st=tombs[d.sheet]||(tombs[d.sheet]={});st[id]=Math.max(Number(st[id]||0),fence++)}
  out.palletStates=out.palletStates.map(pair=>{if(!Array.isArray(pair)||pair.length<2||!pair[0]||!pair[1])return pair;const sheet=String(pair[0]),raw=raws.get(sheet)||cloneJsonV82(pair[1])||{};raw.completed=(kept.get(sheet)||[]).sort((a,b)=>(seq(a)-seq(b))||(created(a)-created(b)));raw.nextSeq=Math.max(Number(raw.nextSeq||1),...raw.completed.map(g=>(Number(g.seq)||0)+1));const repaired=repairInventoryRawV86(raw).raw;return [sheet,repaired]});
  if(dropped.length){out.stateFormatV132=132;out.globalBoxNoRepairV132={removed:dropped.length,at:Date.now()};out.stateRevisionV78=Math.max(Number(out.stateRevisionV78||0),fence);out.savedAt=Date.now()}
  return {state:out,changed:dropped.length>0,removed:dropped.length};
}
function sanitizeStateV132(state){const a=sanitizeStateV86(state),b=sanitizeGlobalBoxNoUniqueV132(a.state);return {state:b.state,changed:!!a.changed||!!b.changed,removed:Number(a.removed||0)+Number(b.removed||0),repaired:Number(a.repaired||0),globalRemoved:Number(b.removed||0)}}
...
app.get('/api/coupang-shared/state', coupangAuth, (req, res) => {
  ...
  const clean=sanitizeStateV132(row.state);
...
app.put('/api/coupang-shared/state', coupangAuth,
  ...
  (req, res) => {
    try {
      ...
      state=sanitizeStateV132(state).state;
...
app.get('/api/coupang-shared/projects/:projectId/state',coupangAuth,(req,res)=>{const found=getProjectOr404V18(req,res);if(!found)return;let row=readJsonSafe(found.paths.state);if(!row)return res.status(404).json({ok:false,error:'저장된 작업 상태가 없습니다.'});const clean=sanitizeStateV132(row.state);if(clean.changed){row={...row,updatedAt:Date.now(),state:clean.state};backupFileV46(found.paths.state);writeAtomic(found.paths.state,JSON.stringify(row))}res.set('Cache-Control','no-store');res.set('X-Updated-At',String(row.updatedAt||0));res.json(row)});
...
app.use('/api/coupang-shared',(req,res,next)=>{
  ...
  if(part==='state'){
    if(req.method==='GET'){let st=readJsonFileV48(paths.state,null);if(!st)return res.status(404).json({ok:false,error:'저장 상태가 없습니다.'});const clean=sanitizeStateV132(st.state);if(clean.changed){st={...st,updatedAt:Date.now(),state:clean.state};atomicJsonV48(paths.state,st)}res.set('X-Updated-At',String(st.updatedAt||0));return res.json(st)}
    if(req.method==='DELETE'){fs.rmSync(paths.state,{force:true});touch(id);return res.json({ok:true})}
    if(req.method==='PUT')return rawBodyV48(req).then(buf=>{let state={};try{state=JSON.parse(buf.toString('utf8')||'{}')}catch(_){state=req.body||{}}const existing=readJsonFileV48(paths.state,null);state=protectManualPalletCloseV77(existing,state);state=sanitizeStateV132(state).state;const updatedAt=Date.now();atomicJsonV48(paths.state,{ok:true,updatedAt,state});touch(id);res.json({ok:true,updatedAt,manualCloseGuardV77:true})}).catch(e=>res.status(400).json({ok:false,error:e.message}));
  }
  if(part.startsWith('blob/')){const key=part.split('/')[1],info=paths[key];if(!info)return res.status(404).json({ok:false,error:'파일 키 오류'});if(req.method==='GET'){const meta=readJsonFileV48(info.meta,null);if(!meta||!fs.existsSync(info.data))return res.status(404).json({ok:false,error:'저장된 파일이 없습니다.'});res.set('Cache-Control','no-store');res.set('Content-Type',meta.type||'application/octet-stream');res.set('Content-Length',String(fs.statSync(info.data).size));res.set('X-Updated-At',String(meta.updatedAt||0));res.set('X-File-Name',meta.name||'');res.set('X-File-Type',meta.type||'');res.set('X-File-Mode',meta.mode||'');res.set('X-Saved-At',String(meta.savedAt||meta.updatedAt||0));return res.sendFile(info.data)}if(req.method==='DELETE'){fs.rmSync(info.data,{force:true});fs.rmSync(info.meta,{force:true});touch(id);return res.json({ok:true})}if(req.method==='PUT'){const writeBlobV64=()=>{try{const buf=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buf.length)return res.status(400).json({ok:false,error:'빈 파일입니다.'});fs.mkdirSync(paths.dir,{recursive:true});const updatedAt=Date.now(),meta={updatedAt,size:buf.length,name:String(req.get('X-File-Name')||''),type:String(req.get('X-File-Type')||''),mode:String(req.get('X-File-Mode')||''),savedAt:Number(req.get('X-Saved-At')||updatedAt)};fs.writeFileSync(info.data,buf);atomicJsonV48(info.meta,meta);if(key==='source'){fs.rmSync(paths.state,{force:true});fs.rmSync(paths.workbookSnapshot.data,{force:true});fs.rmSync(paths.workbookSnapshot.meta,{force:true})}touch(id);return res.json({ok:true,updatedAt,size:buf.length,accountScoped:true})}catch(e){return res.status(500).json({ok:false,error:e.message})}};if(Buffer.isBuffer(req.body))return writeBlobV64();return accountCoupangBlobRawV64(req,res,err=>{if(err)return res.status(err.type==='entity.too.large'?413:400).json({ok:false,error:err.message||'파일 업로드를 읽지 못했습니다.'});return writeBlobV64()})}}
  next();
});
